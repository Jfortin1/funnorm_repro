...
rocDir <- paste0(sampleSizeDir, "/rocData_noob")
...
for (i in 1:5){
	...
	mean.list <- return.mean(matrix.x=matrix.x,matrix.y=matrix.y)
	mean <- mean.list$mean
	x <- 1-mean.list$grid
	up = mean.list$up
	down =mean.list$down
	...
	setwd(rocPlotDir)
	save(totalRocData, file=paste0("data.noob.n.",n.vector[i],".Rda"))
	pdf(paste0("roc.plot.noob.n.",n.vector[i],".pdf"))
