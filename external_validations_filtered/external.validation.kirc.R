...
setwd(svaFunnormDir)
load("sva_funnorm_dmps_dis_kirc.Rda")
dmps450k <- c(dmps450k,dmps)
...
setwd(kirc27kDir)
load("dmps_27k_kirc_plate_adjusted.Rda")
...
save(overlapData1K, file="overlapData1K_kirc_dis.Rda")
...
rocdata_27k_kirc_dis <- generateROCDataFromExternalTruth(p.list, dmps450k)
...
# Need to add dmps for sva, sva+funnorm and combat
setwd(svaDir)
load("sva_dmps_val_kirc.Rda")
dmps450k <- c(dmps450k,dmps)

setwd(svaFunnormDir)
load("sva_funnorm_dmps_val_kirc.Rda")
dmps450k <- c(dmps450k,dmps)
...
setwd(externalValDir)
overlapData1K <- generateOverlapData(dmps27k, dmps450k, step=10)
save(overlapData1K, file="overlapData1K_kirc_val.Rda")
...
setwd(scriptDir)
source("generateROCDataFromExternalTruth.R")
rocdata_27k_kirc_val <- generateROCDataFromExternalTruth(p.list, dmps450k)
	
setwd(externalValDir)	
save(rocdata_27k_kirc_val , file="rocdata_27k_kirc_val.Rda")
