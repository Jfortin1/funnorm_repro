...
#gg <- rgset_aml[,1:15]
#covs <- matrix(sample(100), nrow=15, ncol=2)

#covs <- matrix(sample(100), nrow=ncol(rgset_aml), ncol=2)
#try <- preprocessFunnorm_extended(rgSet = rgset_aml, covs = covs, nPCs = 2)


preprocessFunnorm_extended <- function(rgSet, nPCs=2, methylumiObject=NULL, covs=NULL, sex = NULL, verbose = TRUE, bg.corr = FALSE) {
    #minfi:::.isRG(rgSet)
    ...
    # Noob background correction iomplemented in methylumi: 
    if (!is.null(methylumiObject) && bg.corr){
