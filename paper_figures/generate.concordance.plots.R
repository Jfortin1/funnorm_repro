...
vectorSVARUV <- c(11,1, 9,12,8)
colorsRUV <- c("black", "gray70", "deeppink3", "deeppink3", "darkslategray4", "darkslategray4")
colorsRUV <- c("black", "gray70", "darkslategray4","deeppink3")
namesRUV <- c("Raw", "SVA", "Funnorm", "Funnorm + SVA", "RUV", "Funnorm + RUV")
...
create.overlap.plots.kirc.27k <- function(input.dir = "/Users/Jean-Philippe/funnorm_repro", output.dir = "/Users/Jean-Philippe/funnorm_repro/repro_document/figures_repro", 
	...
	if (save) {
		...
		printOverlapFromData(overlapSVA, xcutoff = 15000, ycutoff = 0.3, main = "", colors = c(combat.color, colorsSVA), names = c("ComBat", namesSVA), lty = c(1, 
			1, 1, 1, 2), lwd = c(3, 3, 3, 3, 2))
		dev.off()

		pdf("KIRC_Concordance_27kValidation_MainMethods_SVARUV.pdf", width = 5, height = 5)
		printOverlapFromData(overlapRUV, xcutoff = 15000, ycutoff = 0.3, main = "", colors = c(combat.color, colorsRUV), names = c("ComBat", namesRUV), lty = c(1, 
			1, 1, 1, 2, 1, 2), lwd = c(3, 3, 3, 3, 2, 3, 2))
		dev.off()
	}
	...
	if (print) {
		...
		#	1, 1, 1, 2), lwd = c(3, 3, 3, 3, 2))
		printOverlapFromData(overlapRUV, xcutoff = 15000, ycutoff = 0.3, main = "", colors = c(combat.color, colorsRUV), names = c("ComBat", namesRUV), lty=rep(1,10), lwd = c(3, 3, 3, 3, 3, 3, 2))
	}
...
create.overlap.plots.aml.27k <- function(input.dir = "/Users/Jean-Philippe/funnorm_repro", output.dir = "/Users/Jean-Philippe/funnorm_repro/repro_document/figures_repro", 
	...
	if (save) {
		...
		printOverlapFromData(overlapSVA, xcutoff = 25000, ycutoff = 0.3, main = "", colors = c(combat.color, colorsSVA), names = c("ComBat", namesSVA), lty = c(1, 
			1, 1, 1, 2), lwd = c(3, 3, 3, 3, 2))
		dev.off()

		pdf("LAML_Concordance_DisVal_MainMethods_SVARUV.pdf", width = 5, height = 5)
		printOverlapFromData(overlapRUV, xcutoff = 25000, ycutoff = 0.3, main = "", colors = c(combat.color, colorsRUV), names = c("ComBat", namesRUV), lty = c(1, 
			1, 1, 1, 2, 1, 2), lwd = c(3, 3, 3, 3, 2, 3, 2))
		dev.off()
		...
		printROCFromROCData(rocSVA, xcutoff = 0.1, main = "", colors = c(combat.color, colorsSVA), names = c("ComBat", namesSVA), lty = c(1, 1, 1, 1, 2), lwd = c(3, 
			3, 3, 3, 2))
		dev.off()

		pdf("LAML_CompleteROC_MainMethods_27k_SVA.pdf", width = 5, height = 5)
		printROCFromROCData(rocSVA, xcutoff = 1, main = "", colors = c(combat.color, colorsSVA), names = c("ComBat", namesSVA), lty = c(1, 1, 1, 1, 2), lwd = c(3, 
			3, 3, 3, 2))
		dev.off()

		pdf("LAML_PartialROC_MainMethods_27k_SVARUV.pdf", width = 5, height = 5)
		printROCFromROCData(rocRUV, xcutoff = 0.1, main = "", colors = c(combat.color, colorsRUV), names = c("ComBat", namesRUV), lty = c(1, 1, 1, 1, 2, 1, 2), 
			lwd = c(3, 3, 3, 3, 2, 3, 2))
		dev.off()

		pdf("LAML_CompleteROC_MainMethods_27k_SVARUV.pdf", width = 5, height = 5)
		printROCFromROCData(rocRUV, xcutoff = 1, main = "", colors = c(combat.color, colorsRUV), names = c("ComBat", namesRUV), lty = c(1, 1, 1, 1, 2, 1, 2), 
			lwd = c(3, 3, 3, 3, 2, 3, 2))
		dev.off()
	}
	...
	if (print) {
		...
		#	1, 1, 1, 2), lwd = c(3, 3, 3, 3, 2))
		printOverlapFromData(overlapRUV, xcutoff = 25000, ycutoff = 0.3, main = "", colors = c(combat.color, colorsRUV), names = c("ComBat", namesRUV), lty = rep(1,10), lwd = c(3, 3, 3, 3, 3, 3, 2))
		printROCFromROCData(roc4, xcutoff = 0.1, main = "", colors = colors4, names = names4, lty = rep(1, 10), lwd = rep(3, 10))
		printROCFromROCData(roc4, xcutoff = 1, main = "", colors = colors4, names = names4, lty = rep(1, 10), lwd = rep(3, 10))
		printROCFromROCData(roc7, xcutoff = 0.1, main = "", colors = colors7, names = names7, lty = rep(1, 10), lwd = rep(2.5, 10))
		printROCFromROCData(roc7, xcutoff = 1, main = "", colors = colors7, names = names7, lty = rep(1, 10), lwd = rep(2.5, 10))
		...
		#	3, 3, 3, 2))
		printROCFromROCData(rocRUV, xcutoff = 0.1, main = "", colors = c(combat.color, colorsRUV), names = c("ComBat", namesRUV), lty = rep(1,10), 
			lwd = c(3, 3, 3, 3, 3, 3, 2))
		printROCFromROCData(rocRUV, xcutoff = 1, main = "", colors = c(combat.color, colorsRUV), names = c("ComBat", namesRUV), lty = rep(1,10), 
			lwd = c(3, 3, 3, 3, 3, 3, 2))
	}
